#!/usr/bin/env python3
"""Kraken inquiry pilot: append-only record, read-only evidence, review, projection."""
...
MID='mission.kraken-inquiry-001'; CID='run.kraken-inquiry-001'; TERMINAL={'completed','rejected','cancelled','failed'}
...
class Record:
 ...
 def events(self): return [dict(r)|{'payload':json.loads(r['payload_json'])} for r in self.db.execute('SELECT * FROM mission_events WHERE mission_id=? ORDER BY seq',(MID,))]
 def status(self):
 ...
 def append(self,event_type,payload,event_id=None):
  canonical=json.dumps(payload,sort_keys=True,separators=(',',':')); event_id=event_id or stable('missionevent',{'type':event_type,'payload':payload})
  ...
  seq=self.db.execute('SELECT COALESCE(MAX(seq),0)+1 FROM mission_events WHERE mission_id=?',(MID,)).fetchone()[0]
  self.db.execute('INSERT INTO mission_events VALUES(?,?,?,?,?,?,?)',(event_id,MID,CID,seq,event_type,canonical,now())); self.db.commit(); return event_id

def envelope(status): return {'schema_version':'monad.mission.v0.1','mission_id':MID,'correlation_id':CID,'parent_mission_id':None,'kind':'inquiry','objective':'What does the verified evidence justify about contact K-1?','requested_by':{'id':'lieutenant.cgl','authority':'human-command'},'status':status,'created_at':now(),'updated_at':now(),'input_refs':[],'constraints':['Unknown identity or approach alone does not establish hostility.'],'data':{}}
def create(r): r.append('mission_created',envelope('created'),'missionevent.kraken.created')
def snapshot(url):
 with urllib.request.urlopen(url.rstrip('/')+'/snapshot',timeout=6) as x: return json.load(x)
def execute(r,url):
 if r.status()=='absent': create(r)
 if r.status()=='created': r.append('status_changed',envelope('running'),'missionevent.kraken.running')
 ...
 ev={'schema_version':'monad.evidence.v0.1','evidence_id':'evidence.fleetcore.kraken-watch','source_system':'fleetcore','source_id':f"snapshot.tick.{s.get('tick')}",'locator':{'kind':'api','value':'/fleetcore-ws/snapshot#watch_events[K-1]'},'content_sha256':None,'observed_at':now(),'recorded_at':now(),'classification':'verified-state','claim':claim,'producer':'fleetcore-reader','supersedes_evidence_id':None}
 r.append('evidence_cited',ev,'missionevent.kraken.evidence')
