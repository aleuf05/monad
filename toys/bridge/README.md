...
The Status Board's **Command Authority** row surfaces whether that token actually worked, without opening Fleet Motion's own panel to check: "Granted," "Read-Only," or "N/A (Local Sim)" when not live at all. Fleet Motion writes this as `liveCommandAuthority` on the shared `MonadFleetState` contract (`toys/shared/fleet-state.js`) the same way it already writes `dataSource` — Bridge only reads it, on the same 1s poll/`storage`-event path it already used for the Data Source row.
