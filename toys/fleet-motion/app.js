...
const SCENARIO_PRESETS = {
  ...
  escort_screen_drill: {
    ...
    label: "Escort Screen Drill",
    variantName: "escort_screen_drill",
    settings: {
    ...
    waypoints: [],
    spawnThreat: true,
    notePrompt: "Threat drill. Judge whether escorts visibly screen MONAD before contact breach."
  },
  waypoint_threading: {
...
const spawnThreatButton = document.querySelector("#spawnThreatButton");
const resetThreatButton = document.querySelector("#resetThreatButton");
...
function updateFleetMarkers() {
  ...
  courseLine.setLatLngs(
    routeQueue.length
      ? [
          [flagship.lat, flagship.lng],
          ...routeQueue.map((point) => [point.lat, point.lng])
        ]
      : []
  );
  updateThreatLayer();
...
function showBlockedRoute(target, start = flagship) {
...
function buildScenarioArtifact() {
  ...
  return {
    ...
    parameters: {
      ...
      timeWarp,
      threatSpeedKmh: THREAT_SPEED_KMH,
      threatInterceptRadiusKm: THREAT_INTERCEPT_RADIUS_KM,
      threatBreachRadiusKm: THREAT_BREACH_RADIUS_KM
    },
    fleet: {
      flagship: {
        name: "MONAD",
...
function updateStatus() {
  ...
  threatStatus.textContent = threatLabel();
  scoreValue.textContent = `${score.neutralized} neutralized / ${score.breaches} breaches`;
  ...
  spawnThreatButton.disabled = Boolean(threat);
  resetThreatButton.disabled = !threat && score.neutralized === 0 && score.breaches === 0;
...
function setDestination(position) {
  ...
  if (!navigationCheck.clear) {
    ...
    showBlockedRoute(navigationCheck.end || nextDestination, navigationCheck.start || flagship);
...
function addWaypoint(position) {
  ...
  if (!navigationCheck.clear) {
    ...
    showBlockedRoute(waypoint, start);
...
function validateEditedWaypoints(actionMessage) {
  ...
  if (!navigationCheck.clear) {
    ...
    showBlockedRoute(navigationCheck.end || waypoints[waypoints.length - 1], navigationCheck.start || flagship);
    return false;
  }
...
spawnThreatButton.addEventListener("click", spawnThreat);

resetThreatButton.addEventListener("click", () => resetThreatDrill());
