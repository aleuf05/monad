...
allowed=(docs/doctrine docs/wardroom docs/workflows admiralty/archive/canon scripts/wardroom-continuity-pass.sh scripts/wardroom-handoff.sh scripts/wardroom-chat-handoff.sh scripts/captain-publish-canon.sh tools/wardroom)
...
outside="$(git diff --cached --name-only | awk '
  !($0 ~ /^docs\/doctrine\// || $0 ~ /^docs\/wardroom\// || $0 ~ /^docs\/workflows\// || $0 ~ /^admiralty\/archive\/canon\// || $0 ~ /^scripts\/(wardroom-continuity-pass|wardroom-handoff|wardroom-chat-handoff|captain-publish-canon)\.sh$/ || $0 ~ /^tools\/wardroom\//) { print }
')"
if [[ -n "$outside" ]]; then
