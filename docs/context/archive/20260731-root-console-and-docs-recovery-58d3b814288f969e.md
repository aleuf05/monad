...
> Compact projection, not project canon. Generated `2026-07-31T13:31:44+00:00` from cited
> repository sources. Input digest: `58d3b814288f969e`.
...
A prior session crashed mid-redesign with uncommitted work; this session recovered state from disk and the host (not from chat memory), verified everything described was already fully built and live, then committed and pushed it as agent/living-world-intake-v0-1 commit 52a1d10. Nothing is currently blocked or in flight.
...
- Network isolation (LAN-only Caddy site block), not password-gating the public domain, is the chosen security boundary for the private Admiral-Captain conference channel -- an explicit, narrow, discussed exception to the CLAUDE.md 'no strange ports' policy, not a precedent for LAN-only being fine by default.
- The Root Console splash page is intentionally a non-functional preview shipped alongside the real, functional app rather than gating the real app behind an unfinished redesign -- the Admiral gets both the working tool today and visibility into where the UI is headed.
- Historical/superseded documentation is marked and cross-linked forward rather than deleted, per docs/history/README.md's reading rules -- consistent with this session's charter and session-packet edits.

## Live and changed surfaces

- tools/chat-captain/ -- new authenticated Codex-backed Captain conversation service.
- console/ -- new LAN-only Root Console (index.html splash preview, app.html real app, assets/js/chat-captain.js).
- scripts/Caddyfile, scripts/chat-captain-web.service, scripts/install-chat-captain-web.sh, cmds.sh -- LAN console deploy and its audit trail; live and matches the running host config.
- README.md -- rewritten front door.
- docs/mission.md, docs/README.md, docs/command-structure.md, docs/safety/README.md, docs/history/README.md -- new documentation-level entry points.
- docs/deployment.md, 001_MONAD_COMMAND_CHARTER_2026-07-15.md, docs/research/MONAD_SESSION_PACKETS_DRAFT_2026-07-31.md -- annotated for the new structure without deleting prior content.
- docs/research/SEMANTIC_ARTIFACT_ENGINEERING.md, docs/research/OPERATIONAL_TOPOLOGY_PACKET_006_2026-07-31.md -- new research material.
...
No committed next action from the Admiral yet. Open threads: (1) confirm with the Admiral whether the Root Console splash's proposed next features (Project Formation, Design, Review, Associative Lab, Record modes; the Harvest Tray Accept/Reject actions; the automatic containment/review queue) should move from mockup to real implementation, and in what order; (2) reconcile with whatever the concurrent Codex doc-archive session lands, since both sessions touched docs/history/ and related files independently; (3) visually verify the Root Console and Chat Captain UI in an actual browser on the LAN once available, since only HTTP/source-level verification has been possible here.
