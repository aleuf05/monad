...
4. **Scope and exclusions** — Static console, dependency-free STL preview, manifest catalog, loopback API, validation, tests, service/Caddy handoff. Native `.io`, `.scm`, and `.ss` sources are accepted; STEP/OpenSCAD/Python import and canonical-world authority are excluded.
5. **Constraints / authority** — Native source execution is isolated by the service (`NoNewPrivileges`, private temp/devices, hidden home, read-only system/repository, output-only write path, and denied network). Safe output names and a 64 KB source ceiling apply; generated assets remain non-canonical.
