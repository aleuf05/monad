...
## When a packet is required vs. a lighter `queue.md` task
...
Use a **`queue.md` task** (see `../queue.md`, protocol in
...
- **`cmd.sh` / `docs/commissioning-handoff.md`** -- privileged (`sudo`)
  ...
  Lieutenant.
- **`queue.md` / `AGENTS.md`** -- non-privileged, lighter synthesis and
  verification tasks. Multiple tasks queued at once, claimed via git's
  own fast-forward check.
