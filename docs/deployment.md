...
- **`web/`, served at `https://cameronlampley.com/monad/`, is the single deploy target.** Do not stand up a second one. Concretely:
  - Never launch an ad hoc local server (`python3 -m http.server <port>`, `npx serve`, etc.) against this repo, even "just to check" a change — `web/` is already live with no deploy step, so verify directly against `https://cameronlampley.com/monad/...` instead. An orphaned instance of exactly this (bound `0.0.0.0`, serving a raw directory listing of the whole repo root including `.git/`) was found running unattended on 2026-07-13 and had to be killed as a live secrets-exposure risk — it wasn't hypothetical.
